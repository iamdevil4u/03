...
const Body = () => {
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            const crimeMatch = text.match(/Crime:\s*([^B]*)/i);
            const bnsMatch = text.match(/BNS Section:\s*([^L]*)/i);
            const judgmentMatch = text.match(/Landmark Judgment:\s*(.*)/i);
            setParsedResult({
                crime: crimeMatch ? crimeMatch[1].trim() : '',
                bns: bnsMatch ? bnsMatch[1].trim() : '',
                ...
                <div id="home" className="hero-text">
                    ...
                    <div className="search-container">
                        <div className="search-bar">
                            ...
                            <button
                                ...
                                // Disable if already loading analysis or if voice recognition not supported
                    ...
                    {parsedResult && (
                        <div className="response-container">
                            ...
                            <p><strong>Crime:</strong> {parsedResult.crime}</p>
                            <p><strong>BNS Section:</strong> {parsedResult.bns}</p>
                            <p><strong>Landmark Judgment:</strong> {parsedResult.judgment}</p>
                            {response?.latency && <p className="latency">Response time: {response.latency}</p>}
                        </div>
                    )}
                </div>
